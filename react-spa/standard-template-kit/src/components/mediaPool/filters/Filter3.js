...
import filter3Payload from "./payloads/filter3Payload.json";
...
export default function Filter3({
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const groups =
          countData?.aggregations?.customAttribute_477?.aggs?.id?.subGroups ||
          countData?.aggregations?.customAttribute_477?.subGroups ||
          [];
  ...
  const mapTree = (nodes, countsMap) => {
    ...
    const mapNode = (n) => {
      ...
      return {
        ...
        value: String(id),
        count: total,
        isChecked: selectedFilter3.includes(String(id)),
        isParentOpen: false,
        children,
      };
    };
  ...
  useEffect(() => {
    ...
    setParents((prev) => applySelectionToTree(prev, selectedFilter3));
  }, [selectedFilter3.join(','), parents.length]);
  ...
  const applySelection = () => {
    ...
    traverse(parents);
    onUpdateSelectedFilter3(values);
  ...
  return (
    <div className="searchFilter filter3">
      <Button className="filterButton" onClick={toggleFilter}>Asset Group</Button>

      {isFilterOpen && (
        <div className="filterDropdown">
          <div className="filterOverlay" onClick={toggleFilter}></div>
          <div className="filterHeader">
            <div className="filterName">Asset Group</div>
