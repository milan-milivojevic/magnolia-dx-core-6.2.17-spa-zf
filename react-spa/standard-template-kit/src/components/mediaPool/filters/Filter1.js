...
import filter1Payload from "./payloads/filter1Payload.json";
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const groups =
          countData?.aggregations?.customAttribute_439?.aggs?.id?.subGroups || [];
  ...
  const mapTree = (nodes, countsMap) => {
    ...
    const mapNode = (n) => {
      ...
      return {
        ...
        value: String(id),
        count: total,
        isChecked: selectedFilter1.includes(String(id)),
        isParentOpen: false,
        children,
      };
    };
...
useEffect(() => {
  ...
  const applySelection = () => {
    ...
    traverse(parents);
    onUpdateSelectedFilter1(values);
  ...
  return (
    <div className="searchFilter filter1">
