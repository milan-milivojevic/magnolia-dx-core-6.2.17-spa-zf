...
const MultiDownloadModalIframe = ({ assetIds = [], isOpen, onClose }) => {
  ...
  const handleDownloadClick = useCallback(async () => {
    ...
    if (!key) {
      alert('Operation key not found from iFrame.');
      return;
    }

    const eligible = Array.isArray(eligibleRef.current) ? eligibleRef.current : [];
    if (!eligible.length) {
      alert('No asset is currently ready for download. Check if you have selected the required items and/or accepted the licenses.');
      return;
    }
    ...
    try {
      ...
      console.error(e);
      setError('Unexpected error during download.');
      setBusy(false);
      setPhase('error');
    }
            ...
            <div className="multiFooter">
              <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                ...
                <button
                  ...
                  title={downloadDisabledByLicense ? 'You need to accept the license for at least one asset.' : undefined}
                >
                  {busy ? (
                    <>
